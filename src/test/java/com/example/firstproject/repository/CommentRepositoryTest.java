...
class CommentRepositoryTest {
    ...
    void findByNickname() {
        ...
        /* null의 모든 댓글 조회(특정 닉네임의 입력값이 null일 때) */
        {
            // 1. 입력 데이터 준비
            ...
            List<Comment> comments = commentRepository.findByNickname(nickname);
            // 3. 예상 데이터
            List<Comment> expexted = Arrays.asList();
            // 4. 비교 및 검증
            assertEquals(expexted.toString(), comments.toString(), "null의 모든 댓글을 출력");
        }
        /* null의 모든 댓글 조회(특정 닉네임의 입력값이 없을 때) */
        {
            // 1. 입력 데이터 준비
            String nickname = "";
            // 2. 실제 데이터
            ...
            List<Comment> expexted = Arrays.asList();
            // 4. 비교 및 검증
            assertEquals(expexted.toString(), comments.toString(), "\"\"의 모든 댓글을 출력");
        }
    }
}